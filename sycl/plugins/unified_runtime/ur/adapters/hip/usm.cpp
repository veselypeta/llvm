...
ur_result_t USMFreeImpl(ur_context_handle_t Context, void *Pointer) {
  ...
  try {
    ...
    if (IsManaged || Type == CU_MEMORYTYPE_DEVICE) {
      ...
      // with cuMemFree
      //Result = UR_CHECK_ERROR(hipFree((CUdeviceptr)Pointer));
      Result = UR_CHECK_ERROR(hipFree(Pointer));
    } else {
      // Memory allocated with cuMemAllocHost must be freed with cuMemFreeHost
      Result = UR_CHECK_ERROR(hipFreeHost(Pointer));
    }
    */
    hipPointerAttribute_t hipPointerAttributeType;
    Result =
        UR_CHECK_ERROR(hipPointerGetAttributes(&hipPointerAttributeType, Pointer));
    ...
    if (Type == hipMemoryTypeHost) {
      //Result = UR_CHECK_ERROR(hipFreeHost(Pointer));
      Result = UR_CHECK_ERROR(hipHostFree((hipDeviceptr_t)Pointer));
    }
...
ur_result_t USMDeviceAllocImpl(void **ResultPtr, ur_context_handle_t Context,
  ...
  try {
    ScopedContext Active(Context->getDevice());
    //UR_CHECK_ERROR(hipMalloc((CUdeviceptr *)ResultPtr, Size));
...
ur_result_t USMSharedAllocImpl(void **ResultPtr, ur_context_handle_t Context,
  ...
  try {
    ScopedContext Active(Context->getDevice());
    //UR_CHECK_ERROR(hipMallocManaged((CUdeviceptr *)ResultPtr, Size,
                                     //CU_MEM_ATTACH_GLOBAL));
    UR_CHECK_ERROR(hipMallocManaged(ResultPtr, Size,
                                     hipMemAttachGlobal));
  } catch (ur_result_t Err) {
    return Err;
  }
